...
s3_bucket_name_src = 'src-bucket'
s3_bucket_name_trg = 'trg-bucket'
meta_key = 'meta_key'
...
def create_buckets(s3_mock_resource):
    s3_mock_resource.create_bucket(Bucket=s3_bucket_name_src,
                                   CreateBucketConfiguration={
                                       'LocationConstraint': 'eu-central-1'})
    s3_mock_resource.create_bucket(Bucket=s3_bucket_name_trg,
                                   CreateBucketConfiguration={
                                       'LocationConstraint': 'eu-central-1'})


@pytest.fixture(scope='module', autouse=True)
def src_bucket(s3_mock_resource, create_buckets):
    yield s3_mock_resource.Bucket(s3_bucket_name_src)


@pytest.fixture(scope='module', autouse=True)
def trg_bucket(s3_mock_resource, create_buckets):
    yield s3_mock_resource.Bucket(s3_bucket_name_trg)
...
@pytest.fixture(scope='module', autouse=True)
def s3_bucket_src():
    s3_bucket_src = S3BucketConnector(s3_access_key,
                                      s3_secret_key,
                                      s3_endpoint_url,
                                      s3_bucket_name_src)
    s3_bucket_src.write_df_to_s3(df_src.loc[0:0],
                                 '2021-04-15/2021-04-15_BINS_XETR12.csv', 'csv')
    s3_bucket_src.write_df_to_s3(df_src.loc[1:1],
                                 '2021-04-16/2021-04-16_BINS_XETR15.csv', 'csv')
    s3_bucket_src.write_df_to_s3(df_src.loc[2:2],
                                 '2021-04-17/2021-04-17_BINS_XETR13.csv', 'csv')
    s3_bucket_src.write_df_to_s3(df_src.loc[3:3],
                                 '2021-04-17/2021-04-17_BINS_XETR14.csv', 'csv')
    s3_bucket_src.write_df_to_s3(df_src.loc[4:4],
                                 '2021-04-18/2021-04-18_BINS_XETR07.csv', 'csv')
    s3_bucket_src.write_df_to_s3(df_src.loc[5:5],
                                 '2021-04-18/2021-04-18_BINS_XETR08.csv', 'csv')
    s3_bucket_src.write_df_to_s3(df_src.loc[6:6],
                                 '2021-04-19/2021-04-19_BINS_XETR07.csv', 'csv')
    s3_bucket_src.write_df_to_s3(df_src.loc[7:7],
                                 '2021-04-19/2021-04-19_BINS_XETR08.csv', 'csv')
    s3_bucket_src.write_df_to_s3(df_src.loc[8:8],
...
class TestXetraETLMethods(unittest.TestCase):
    ...
    def test_extract_no_files(self, s3_bucket_src, s3_bucket_trg):
        """
        ...
        with patch.object(MetaProcess, "return_date_list",
            ...
            xetra_etl = XetraETL(s3_bucket_src, s3_bucket_trg,
                                 meta_key, source_config, target_config)
